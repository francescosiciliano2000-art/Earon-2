...
class HearingDispositionDialog extends StatefulWidget {
  final Map<String, dynamic> hearing; // hearing row già caricato (hearing_id, matter_id, type, ends_at, time, courtroom, notes)
  const HearingDispositionDialog({super.key, required this.hearing});

  @override
  State<HearingDispositionDialog> createState() => _HearingDispositionDialogState();
}
...
class _HearingDispositionDialogState extends State<HearingDispositionDialog> {
  ...
  Future<void> _confirm() async {
    ...
    if ((_outcome == 'rinviata' || _outcome == 'in_riserva' || _outcome == 'a_sentenza')) {
    ...
    try {
      ...
      final type = (h['type']?.toString() ?? '').isEmpty ? null : h['type']?.toString();
      final courtroom = (h['courtroom']?.toString() ?? '').isEmpty ? null : h['courtroom']?.toString();
      final notes = (h['notes']?.toString() ?? '').isEmpty ? null : h['notes']?.toString();
      ...
      if (_outcome == 'rinviata') {
        ...
        await _sb
            .from('hearings')
            .update({'adjourned_at': adj.toIso8601String()})
        ...
        final startsToday = DateTime(now.year, now.month, now.day).toIso8601String();
        ...
        final msg = 'Udienza rinviata con successo al ${dd.day.toString().padLeft(2, '0')}-${dd.month.toString().padLeft(2, '0')}-${(dd.year % 100).toString().padLeft(2, '0')} alle ore $hhmm';
        Navigator.of(context).pop({'outcome': _outcome, 'message': msg});
        return;
      }

      if (_outcome == 'in_riserva' || _outcome == 'a_sentenza') {
        final title = _outcome == 'in_riserva' ? 'Verifica Scioglimento' : 'Verifica Provvedimento';
  ...
  Widget build(BuildContext context) {
    ...
    return AppDialogContent(
      ...
      children: [
        ...
        // Corpo: altezza dinamica senza vincoli fissi
        Column(
          ...
          children: [
            if (_step == 1) ...[
              ...
              Column(
                children: [
                  _OutcomeVerticalCard(
                    title: 'Rinviata',
                    description: 'Segna l’udienza come rinviata e pianifica una nuova data. L’udienza corrente viene aggiornata (adjourned_at) e ne viene creata una nuova.',
                    value: 'rinviata',
                  ...
                  _OutcomeVerticalCard(
                    title: 'In riserva',
                    description: 'Crea un adempimento di “Verifica Scioglimento” collegato alla pratica per la data scelta.',
                    value: 'in_riserva',
                  ...
                  _OutcomeVerticalCard(
                    title: 'A sentenza',
                    description: 'Crea un adempimento di “Verifica Provvedimento” collegato alla pratica per la data scelta.',
                    value: 'a_sentenza',
              ...
              AppDialogDescription('Creazione adempimenti: scegli la data per creare l’attività correlata alla pratica.'),
              SizedBox(height: su),
              CalendarRange(
                value: _range,
            ...
            if ((matterId).isEmpty) ...[
              ...
              Alert(
                ...
                child: const AlertDescription(
                  Text('Attenzione: questa udienza non è collegata ad alcuna pratica. Alcune azioni potrebbero non essere disponibili.'),
                ),
              ),
            ],
          ],
          ...
          children: [
            ...
            if (_step == 2)
              AppButton(
                ...
                onPressed: _saving ? null : () => setState(() { _step = 1; _resetStep2State(); }),
